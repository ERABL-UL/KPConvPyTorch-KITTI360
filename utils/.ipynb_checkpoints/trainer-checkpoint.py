...
#                    torch.save(save_dict, checkpoint_path)
            ...
            # Validation
            net.eval()
            self.validation(net, val_loader, config, self.epoch, save_dict)
            net.train()
    ...
    def validation(self, net, val_loader, config: Config, epoch, save_dict):
    ...
    def slam_segmentation_validation(self, net, val_loader, config, epoch, save_dict = None, debug=True):
        ...
        if config.saving:
            ...
            with open(epoch_ALL_IoUs_file, "a") as file:
                to_write = ""
                IoUsTxt = ""
                Val_IoUsTxt = ""
